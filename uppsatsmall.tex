...
alfanumeriska tecken \citep{durtva:2017}. Ifall en rapport bryter mot denna regel 
...
Sparv använder verktyget hfst-sweNER \citep{borin:2016} för att identifiera
...
eller delar av den faller in under någon av följande huvudkategorier \citep{kokkinakis:2014}:
...
Den första är en regel som Per gick \cite{perintervju:2019} under vårt andra möte.
...
ett speciellt sätt i rapporter \citep{rfsip}, t.ex. datum ska enbart 
...
\item \textbf{Nominalkvot (NK)}: Mäter informationstätheten i text med följande
  formel: $$NK=(Substantiv + Prepositioner + Particip) / (Pronomen + Verb + Adverb)$$
...
\item \textbf{Ordvariationsindex (OVIX)}: Mäter hur många unika ord som finns i
  texten med formeln: $$OVIX=Unika Ord / Ord *100$$
  Enligt \cite{abrahamsson:2011} är
...
\item \textbf{Läsbarhetsindex (LIX)}: Mäter hur avancerad en text är med
  ...
  $$LIX=(Ord / Meningar) + LångaOrd * 100 / Ord)$$
